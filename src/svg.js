...
export function sheetSvg(sheet, parts, opts) {
  ...
  out.push(`<g id="TEXT" fill="#d0021b" stroke="none">${texts.map(t => {
    const x = t.x.toFixed(2), y = t.y.toFixed(2);
    const tr = t.rot ? ` transform="rotate(${t.rot} ${x} ${y})"` : '';
    const family = `${t.font || 'Arial'}, Helvetica, sans-serif`;
    return `<text x="${x}" y="${y}" font-size="${t.size}" font-family="${escapeXml(family)}" text-anchor="middle"${tr}>${escapeXml(t.text || '')}</text>`;
  }).join('')}</g>`);
  if (labels) {
